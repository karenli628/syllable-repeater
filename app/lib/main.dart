...
final managedTempSessionProvider = Provider<ManagedTempSession?>((ref) => null);

Future<void> main() async {
  WidgetsFlutterBinding.ensureInitialized();
  final basePaths = SidecarPaths.current();
  final tempSession = await ManagedTempSession.start(
    rootDirectory: Directory(basePaths.tempDirectory),
  );
  SidecarPaths.useManagedTempDirectory(tempSession.directory.path);
  final paths = SidecarPaths.current();
  final overrides = <Override>[
    managedTempSessionProvider.overrideWith((ref) {
      ref.onDispose(() {
        SidecarPaths.clearManagedTempDirectory();
        unawaited(tempSession.dispose());
      });
      return tempSession;
    }),
  ...
  if (paths.missingPaths().isEmpty) {
    final durationProbe = FfprobeDurationProbe(
    ...
    overrides
      ...
      ..add(audioDurationProbeProvider.overrideWithValue(durationProbe))
      ..add(
        audioImportReaderProvider.overrideWithValue(
          DartIoAudioImportReader(durationProbe: durationProbe),
        ),
      );
  }
...
class _AppRoot extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ref.watch(managedTempSessionProvider);
    return MaterialApp(
