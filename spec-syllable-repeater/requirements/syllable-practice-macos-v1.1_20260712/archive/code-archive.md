...
| 編號 | 問題 | 影響 | 狀態 |
|------|------|------|------|
| C-101 | v1.1 unsigned zip 已重打（`dist/`，529MB，SHA `e8e60b06…83a91c`），使用者 2026-07-16 定案「保持 bundled 但不對外散布」 | 僅供本機使用，不上 GitHub Release、不分發第三方 | ✅ **關閉**（決策定案，見 `THIRD_PARTY_NOTICES.md`） |
| C-105 | htdemucs 預訓練模型權重授權查證：Facebook demucs 主 LICENSE = MIT、README 無獨立模型聲明、HF 無官方 model card；訓練資料 MUSDB HQ = CC BY-NC-SA 4.0（灰色地帶） | 使用者定案「保持 bundled 但不對外散布」 | ✅ **關閉**（決策定案，未來意圖改變需重新檢視） |
| C-106 | 頂層 `step up your coding skills to a new level.mp3`（YouTube 擷取的 5 秒音檔）於初始 commit（`3c00b15`, 2026-07-06）進版控，已在 public 遠端 | 著作權侵權風險 | ✅ **關閉**（2026-07-16 完成 filter-branch history rewrite ＋ force-with-lease push；遠端當前 SHA `539ad61…`；`.git/` 從 56M→28M；GitHub raw 404、tree 抽驗 0 命中；repo 0 fork/0 star/0 watcher，無擴散風險） |
