...
- **重要邏輯**：
  ...
  7. 使用者按「匯出標籤」→ 另存 `.abolabel` 檔（zip＋JSON：含音檔指紋、Segment 清單、schema 版本號、**當時人聲分離開關狀態**——重載時不需重跑分離，v1.1-r1/O2）；
...
- **重要邏輯**：
  ...
  4. **堆疊組塊（v1.1-r1/F1 定案手勢）**：長按積木拖曳（如 iPhone 主畫面 App），**疊放到同列另一積木上即合成一個組塊**（如 iPhone App 資料夾；例：`rain` 疊上 `itll` 再疊 `rain` → `rain+itll+rain`）；**組塊內可再滑動調整順序**；組塊視為單一設定單位；疊錯了按本區撤銷回復再重疊（如修正為 `[itll, rain, itll+rain]`）；
...
- **重要邏輯**：
  ...
  2. 使用者勾選（＝該次明示同意）→ 錄音 PCM 存入 `RecordingBuffer`（記憶體或 App 暫存目錄，含建立時間與存活時限，**10 分鐘**（v1.1-r1/O4 定案），**不開放於設定頁調整**）；
  3. 回聽：練習頁顯示暫存清單（時間＋所屬步驟），點擊播放；**可手動逐筆刪除，不必等時限**（v1.1-r1/O4）；
  3a. **切換練習步驟即清**（v1.1-r1/O4）：使用者切換至另一個練習步驟時，前一步驟的暫存自動刪除；同一步驟重錄並勾選暫存時，新錄音覆蓋舊暫存；
...
| AT-18-04 | 邊界（時限兩側） | 暫存時限 10 分鐘（v1.1-r1） | 9 分 59 秒時回聽／10 分 01 秒時回聽 | 前者可播；後者已自動清除，清單顯示空 |
| AT-18-07 | 正常（切步即清，v1.1-r1） | 第 3 步暫存 1 筆後切換至第 4 步練習 | 檢查暫存清單與目錄 | 第 3 步暫存已自動刪除；第 4 步重錄勾暫存→新檔覆蓋，同步驟恆最多 1 筆 |
