...
- **版面設定**：桌面雙欄——左 NavigationRail＋右主工作區；最小視窗 1100×700。啟動預設進入課件庫首頁；首頁主要操作列為左側開啟課件、右側課件文字資訊窗格。
...
#### 1.1 功能概述
- **功能描述**：啟動首頁在同一水平顯示「左側開啟 `.abopack`／右側目前課件文字資訊」，下方顯示今日到期複習清單與全部課件；點擊課件可進練習或編輯。
...
- **使用驅動檢查**：①動機＝先開啟既有課件，再看今天該練什麼；②第一眼＝左側開啟課件，成功後右側立即顯示標題、檔名、音節數、譯文與更新時間；③阻力點＝逾期罪惡感——UI **不顯示「逾期 N 天」字樣**（M7 跨日零懲罰的介面落地），只顯示「可練」。
...
#### 5.1 功能概述
- **功能描述**：勾選步驟（支援全部選取／全部取消）→選路徑→匯出單步/合併 mp3；寫入後驗證實檔存在，完成顯示「在 Finder 顯示」。
...
- **實作要點**：預設全選並提供全部選取／全部取消；未勾選→匯出鈕置灰；匯出中再按→置灰＋提示（`ERR_EXPORT_IN_PROGRESS`，AT-04-05）；寫入完成仍須確認目的檔存在；`ERR_EXPORT_DEST_UNWRITABLE`→就地錯誤、勾選狀態保留。

### 功能點 6：課件儲存/開啟與譯文（pack_translate）

#### 6.1 功能概述
- **功能描述**：課件庫首頁開啟 `.abopack` 並在右側顯示課件資訊；設定頁檔案管理區儲存 `.abopack`，譯文欄手動輸入（永遠可用）＋「自動翻譯」（需 key）。
- **實作位置**：`features/library/`、`features/progress/`、`features/pack_translate/`
...
#### 7.1 功能概述
- **功能描述**：檔案管理（儲存 `.abopack`、匯入/匯出 `.aboprogress`）、Group 歸檔/恢復、提醒三參數、AI key 設定、sidecar 逾時。`.aboprogress` 為 zip＋`progress.json`，不含課件音訊，僅以 lesson id/contentHash 與課件對應。
