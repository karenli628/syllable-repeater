...
| 12 | Dependency Scanning | 依賴掃描：檢查第三方套件已知漏洞 | PARTIAL | `pubspec.yaml` 依賴清單可手動核對；M9 授權白名單掃描腳本 = task 8.2（CT-09）**待實作**；brew FFmpeg 記錄為 dev-only GPL build（需 M9 換 LGPL）；**demucs.cpp（sevagh）LICENSE 2026-07-06 已核，MIT + Eigen MPL-2.0 通過白名單**（memory `decision_demucs_cpp_selected_mit_licence`） | — | task 2.1 部分完成；CT-09 授權掃描腳本待 8.2 實作；whisper.cpp（MIT）與 CMUdict（BSD-like）先前已知白名單通過 |
