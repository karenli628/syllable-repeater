...
| 1.4 | 2026-07-12 | AI | 2026-07-12 | Karen（需求方） | 真 App REQ-01→08 驗收修繕：課件庫改為啟動首頁，首頁左側開啟課件、右側顯示課件文字資訊；設定區集中儲存課件與 `.aboprogress` 備份；匯出增加全部選取/取消；錄音停止後可試聽且磁碟暫存仍依 M10 刪除；`.aboprogress` 實作對齊 zip＋`progress.json` 契約 |
...
- **重要邏輯**：
  1. 前端收集勾選步驟與匯出路徑；預設全選，並提供「全部選取／全部取消」操作（未勾選任何步驟時匯出鈕置灰）；
...
| AT-04-07 | 操作效率 | 開啟含 11 步的匯出視窗，預設全選 | 按「全部取消」後再按「全部選取」 | 第一次 11 步全取消且匯出鈕置灰；第二次 11 步全選，不需逐筆點擊 |
...
| AT-06-06 | 真機生命週期 | macOS 錄音 1.8s，外掛 stop 回傳時 WAV 尚在收尾 | 按停止後播放錄音 | App 等待 WAV 可完整解碼才比對；顯示「播放錄音」且可聽見本次收音；來源錄音檔與播放暫存皆不殘留 |
...
- **首頁與檔案管理**：App 啟動進入課件庫首頁；首頁中間偏左提供「開啟課件」，同一水平右側以文字窗格顯示目前課件資訊。課件儲存移至「設定」的檔案管理區，並與 `.aboprogress` 匯入/匯出並列；`.abopack` 與 `.aboprogress` 仍是兩個獨立檔案，後者只以 lesson id/contentHash 對應前者，不含課件音訊。
